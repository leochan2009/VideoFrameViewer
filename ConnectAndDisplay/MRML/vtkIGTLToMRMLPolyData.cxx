  ...
  if (event == vtkMRMLModelNode::PolyDataModifiedEvent && strcmp(mrmlNode->GetNodeTagName(), "Model") == 0)
    ...
    *igtlMsg = (void*)this->OutPolyDataMessage->GetPackPointer();
    ...
    if (qnode)
      ...
      {
        if (this->StopPolyDataMessage.IsNull())
        {
          this->StopPolyDataMessage = igtl::StopPolyDataMessage::New();
        }
        this->StopPolyDataMessage->SetDeviceName(qnode->GetIGTLDeviceName());
        this->StopPolyDataMessage->Pack();
        *size = this->StopPolyDataMessage->GetPackSize();
        *igtlMsg = this->StopPolyDataMessage->GetPackPointer();
        return 1;
      }
